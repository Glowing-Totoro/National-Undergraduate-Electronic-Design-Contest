...
uint8_t g_usart4_rx_buf[USART4_REC_LEN];
...
uint8_t g_rx_buffer4[RXBUFFERSIZE4];  /* HAL库使用的串口接收缓冲 */
...
void usart_init(uint32_t baudrate)
{
    ...
    HAL_UART_Receive_IT(&g_uart1_handle, (uint8_t *)g_rx_buffer, RXBUFFERSIZE);     /*UART 初始化设置*/
    //串口四初始化，增加回调函数
    ...
    /* 该函数会开启接收中断：标志位UART_IT_RXNE，并且设置接收缓冲以及接收缓冲接收最大数据量 */
    HAL_UART_Receive_IT(&g_uart4_handle, (uint8_t *)g_rx_buffer4, RXBUFFERSIZE4); 
}
...
void HAL_UART_MspInit(UART_HandleTypeDef *huart)
{
    ...
    if (huart->Instance == USART_UX)                            /* 如果是串口1，进行串口1 MSP初始化 */
    {
        ...
        HAL_NVIC_SetPriority(USART_UX_IRQn, 2, 3);              /* 组2，最低优先级:抢占优先级3，子优先级3 */
#endif
    }

		if (huart->Instance == USART4_UX)                            /* 如果是串口1，进行串口1 MSP初始化 */
    {
        USART4_TX_GPIO_CLK_ENABLE();                             /* 使能串口TX脚时钟 */
        USART4_RX_GPIO_CLK_ENABLE();                             /* 使能串口RX脚时钟 */
        USART4_UX_CLK_ENABLE();                                  /* 使能串口时钟 */

        gpio_init_struct.Pin = USART4_TX_GPIO_PIN;               /* 串口发送引脚号 */
        gpio_init_struct.Mode = GPIO_MODE_AF_PP;                /* 复用推挽输出 */
        gpio_init_struct.Pull = GPIO_PULLUP;                    /* 上拉 */
        gpio_init_struct.Speed = GPIO_SPEED_FREQ_HIGH;          /* IO速度设置为高速 */
        HAL_GPIO_Init(USART4_TX_GPIO_PORT, &gpio_init_struct);
                
        gpio_init_struct.Pin = USART4_RX_GPIO_PIN;               /* 串口RX脚 模式设置 */
        gpio_init_struct.Mode = GPIO_MODE_AF_INPUT;    
        HAL_GPIO_Init(USART4_RX_GPIO_PORT, &gpio_init_struct);   /* 串口RX脚 必须设置成输入模式 */
        
#if USART_EN_RX
        HAL_NVIC_EnableIRQ(USART4_UX_IRQn);                      /* 使能USART1中断通道 */
        HAL_NVIC_SetPriority(USART4_UX_IRQn, 0, 1);              /* 组2，最低优先级:抢占优先级3，子优先级3 */
#endif
    }
}
...
void HAL_UART_RxCpltCallback(UART_HandleTypeDef *huart)
{
    ...
    if (huart->Instance == USART4_UX)                    /* 如果是串口4 */
    {
        if ((g_usart4_rx_sta & 0x8000) == 0)             /* 接收未完成 */
        {
            if (g_usart4_rx_sta & 0x4000)                /* 接收到了0x0d（即回车键） */
            {
                if (g_rx_buffer4[0] != 0x0a)             /* 接收到的不是0x0a（即不是换行键） */
                {
                    g_usart4_rx_sta = 0;                 /* 接收错误,重新开始 */
                }
                else                                    /* 接收到的是0x0a（即换行键） */
                {
                    g_usart4_rx_sta |= 0x8000;           /* 接收完成了 */
                }
            }
            else                                        /* 还没收到0X0d（即回车键） */
            {
                if (g_rx_buffer4[0] == 0x0d)
                    g_usart4_rx_sta |= 0x4000;
                else
                {
                    g_usart4_rx_buf[g_usart4_rx_sta & 0X3FFF] = g_rx_buffer4[0];
                    g_usart4_rx_sta++;
...
void USART4_UX_IRQHandler(void)
{
    ...
    /* 此时已经清除&关闭中断 */
    while (HAL_UART_Receive_IT(&g_uart4_handle, (uint8_t *)g_rx_buffer4, RXBUFFERSIZE4) != HAL_OK)     /* 重新开启中断并接收数据 */
    {
        /* 如果出错会卡死在这里 */
    }
